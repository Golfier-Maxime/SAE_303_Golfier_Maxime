...
import { getDatas } from '@/composables/serviceAjax.js'
...
let fields = ref()
fields.value = [
    { key: 'fields.nom_du_musee', label: "Nom", type: "string", sortable: true, sort: 1, filter: "" },
    ...
    { key: 'fields.total', label: "Total de fréquentation dans le musée", type: "number", sortable: true, sort: 1, filter: "" },
]
...
const updateGraph = () => {
    chartOptions.plugins.title.text = "Nombre de musée"
    chartData.labels = getLabels(items.value, 'fields.regions#1')
    chartData.datasets[0].data = countDatas(items.value, chartData.labels, 'fields.regions#1')
